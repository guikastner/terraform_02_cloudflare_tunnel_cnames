...
- Docker Engine/CLI available on the host running Terraform (the `cloudflared` container is managed automatically)
...
2. Fill `terraform.tfvars` with your account/zone IDs, the default origin address, proxy preference, and (optionally) override the tunnel/container name.
...
- `tunnel_name` &mdash; name of the Cloudflare Tunnel + container (defaults to `cloudflarecasaos`).
...
- a CNAME record pointing to `<new_tunnel_id>.cfargotunnel.com`
